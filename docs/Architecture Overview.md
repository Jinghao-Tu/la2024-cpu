...
流水线清空时当周期清空Free List、sRAT之外的所有功能部件，第二周期刷新Free List、sRAT，以确保流水线清空当周期正常退休的指令正确产生效果。这时取回的新指令至多到达取指1段，延迟清空不会对指令执行造成影响；CSR当周期被更改，以确保下一周期取指1段的指令在地址翻译时行为正确；若有Cache/TLB指令退休，流水线将被锁定直到LSU完成对Cache/TLB状态的更改
