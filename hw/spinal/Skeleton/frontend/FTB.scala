...
case class FTB(config: CPUConfig) extends Component {
  val io = new Bundle {
    val nextBase = in(UInt(config.valen bits))
  ...
  val btb = Mem(BTBBundle_1(config), wordCount = config.btbSize) init (Seq.fill(config.btbSize)(BTBBundle_1(config).resetVal))
...
// ------------------------------- predict -------------------------------
  ...
  (0 until config.fetchWidth).map(i => {
    npc(i) := io.nextBase + i |<< config.instLength / 8
  })
...
// ------------------------------- update -------------------------------
  ...
  (0 until config.retireWidth).map(i => {
    ...
    val isJumpInst = io.updateInfo(i).payload.isJumpInst || io.updateInfo(i).payload.isCallInst || io.updateInfo(i).payload.isRetInst
